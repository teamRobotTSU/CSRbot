...
ap.add_argument("-f", "--face", required=True, help="path to face cascade resides")
...
def fdet(gray):
    return fd.detectMultiScale(gray, scaleFactor=1.3, minNeighbors=5, minSize=(64,64))
        ...
        while(True):
            ...
            for (x,y,w,h) in faceRects:
                if w > 0:
                    ...
                    t2 = time.time()
                    if(t2 - t1 > 1):
                        cv2.imwrite("picture%d.jpg" %i, frame)
                        i = i + 1
                        t1  = t2
